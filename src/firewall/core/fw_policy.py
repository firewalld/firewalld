...
class FirewallPolicy:
            ...
            for _egress_zone in _egress_zones:
                for backend in self._fw.enabled_backends():
                    ...
                    for table, chain in (
                        ...
                        # sometimes needs to represent the ingress zone (INPUT,
    ...
    def _get_table_chains_for_zone_dispatch(self, ingress_zone, egress_zone):
        if "HOST" == ingress_zone:
            ...
            # FIXME: If iptables-nft can't match these, then we need this code.
            # It's okay to generate the rules, they'll simply not match. We
            # just need to avoid generating them if iptables-nft will reject
            # adding them.
            """
            # In the postrouting phase, iptables can't match
            if self._fw._firewall_backend == "nftables":
                tc.append(("nat", "POSTROUTING"))
            else:
                if not self._fw.zone.get_zone(ingress_zone).interfaces:
                    tc.append(("nat", "POSTROUTING"))
            # In the prerouting phase, we can't know the egress interface
            if not self._fw.zone.get_zone(egress_zone).interfaces:
