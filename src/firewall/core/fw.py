...
if __name__ == "__main__":
    fw = Firewall()
    fw.start()
    fw.enable_service("ssh")
    fw.enable_service("ipp-client")

    for svc in fw_services.service_list:
        try:
            fw.enable_service(svc.key)
        except Exception, msg:
            print msg
    import time
    ...
    for svc in fw_services.service_list:
        try:
            fw.disable_service(svc.key)
        except Exception, msg:
            print msg

    fw.enable_port(123, "tcp")
#    time.sleep(2)
#    fw.disable_port(123, "tcp")
    #fw.disable_port("124", "tcp")
    fw.enable_icmp_block("echo-request")
#    time.sleep(2)
#    fw.disable_icmp_block("echo-request")
    fw.enable_trusted("eth8")
#    time.sleep(2)
#    fw.disable_trusted("eth8")
    fw.enable_forward_port("eth5", 80, "tcp", 81, None)
#    time.sleep(2)
#    fw.disable_forward_port("eth5", 80, "tcp", 81, None)
    #for i in xrange(80,84):
    #    fw.enable_forward_port("eth5", i, "tcp", 81, None)
    fw.enable_masquerade("eth9")
#    time.sleep(2)
#    fw.disable_masquerade("eth9")
    fw.enable_custom(src_port=161, dst_port=4711, protocol="udp")
#    time.sleep(2)
#    fw.disable_custom(src_port=161, dst_port=4711, protocol="udp")
