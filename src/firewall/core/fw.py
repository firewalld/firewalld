    ...
    def _start(self, reload=False, complete_reload=False):
        ...
        try:
            ...
            if self._firewalld_conf.get("FirewallBackend"):
                value = self._firewalld_conf.get("FirewallBackend")
                if os.path.exists(config.FIREWALLD_DIRECT):
                    log.warning("FirewallBackend overridden to '%s' due to existence of '%s'",
                               self._firewall_backend, config.FIREWALLD_DIRECT)
                    self._firewall_backend = "iptables"
                else:
                    self._firewall_backend = value
