...
class IPSet(io_object.IO_Object):
    ...
    def check_entry(entry, options, ipset_type):
        ...
        if ipset_type == "hash:ip":
            ...
            raise errors.FirewallError(errors.INVALID_IPSET,
                                       "ipset type '%s' not usable" % \
                                       ipset_type)

    def _check_config(self, conf, item):
        if item == "type":
            if conf not in IPSET_TYPES:
                raise errors.FirewallError(errors.INVALID_TYPE,
                                           "'%s' is not valid ipset type" % \
                                           conf)

    def import_config(self, conf):
        for entry in conf[5]:
            IPSet.check_entry(entry, conf[4], conf[3])
        super(IPSet, self).import_config(conf)

# PARSER

class ipset_ContentHandler(io_object.IO_Object_ContentHandler):
