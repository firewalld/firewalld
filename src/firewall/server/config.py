...
class FirewallDConfig(DbusServiceObject):
    ...
    def _addIcmpType(self, obj):
        ...
        config_icmptype = FirewallDConfigIcmpType(
            ...
            self.icmptype_idx,
            self.busname,
            "%s/%d" % (config.dbus.DBUS_PATH_CONFIG_ICMPTYPE, self.icmptype_idx),
        )
    ...
    def _updateIcmpType(self, obj):
        for icmptype in self.icmptypes:
            if (
                icmptype.obj.name == obj.name
                and icmptype.obj.path == obj.path
                and icmptype.obj.filename == obj.filename
            ):
                icmptype.obj = obj
    ...
    def removeIcmpType(self, obj):
        ...
        for policy in self.policy_objects:
            ...
            if "icmp_blocks" in settings and obj.name in settings["icmp_blocks"]:
                ...
                policy.obj = self.config.set_policy_object_config_dict(
                    policy.obj, settings
                )
    ...
    def _updateService(self, obj):
        for service in self.services:
            if (
                service.obj.name == obj.name
                and service.obj.path == obj.path
                and service.obj.filename == obj.filename
            ):
                service.obj = obj
    ...
    def _updateZone(self, obj):
        for zone in self.zones:
            if (
                zone.obj.name == obj.name
                and zone.obj.path == obj.path
                and zone.obj.filename == obj.filename
            ):
                zone.obj = obj
    ...
    def _updatePolicy(self, obj):
        for policy in self.policy_objects:
            if (
                policy.obj.name == obj.name
                and policy.obj.path == obj.path
                and policy.obj.filename == obj.filename
            ):
                policy.obj = obj
    ...
    def _updateIPSet(self, obj):
        for ipset in self.ipsets:
            if (
                ipset.obj.name == obj.name
                and ipset.obj.path == obj.path
                and ipset.obj.filename == obj.filename
            ):
                ipset.obj = obj
    ...
    def _updateHelper(self, obj):
        for helper in self.helpers:
            if (
                helper.obj.name == obj.name
                and helper.obj.path == obj.path
                and helper.obj.filename == obj.filename
            ):
                helper.obj = obj
    ...
    def Set(self, interface_name, property_name, new_value, sender=None):
        ...
        log.debug1(
            "config.Set('%s', '%s', '%s')", interface_name, property_name, new_value
        )
    ...
    def PropertiesChanged(
        ...
        log.debug1(
            "config.PropertiesChanged('%s', '%s', '%s')",
            interface_name,
            changed_properties,
    ...
    def getLockdownWhitelist(self, sender=None):  # pylint: disable=W0613
        return firewall.dbus_utils.deprecated_nop(([], [], [], [0]))
    ...
    def setLockdownWhitelist(self, settings, sender=None):  # pylint: disable=W0613
        return firewall.dbus_utils.deprecated_nop(None)
    ...
    def queryLockdownWhitelistCommand(self, command, sender=None):
        return firewall.dbus_utils.deprecated_nop(False)
    ...
    def getLockdownWhitelistCommands(self, sender=None):  # pylint: disable=W0613
        return firewall.dbus_utils.deprecated_nop([])
    ...
    @dbus_service_method(
        config.dbus.DBUS_INTERFACE_CONFIG, in_signature="s", out_signature="o"
    )
    @dbus_handle_exceptions
    ...
    @dbus_service_method(
        config.dbus.DBUS_INTERFACE_CONFIG, in_signature="sa{sv}", out_signature="o"
    )
    @dbus_handle_exceptions
    ...
    @dbus_service_method(
        config.dbus.DBUS_INTERFACE_CONFIG, in_signature="s", out_signature="s"
    )
    @dbus_handle_exceptions
    ...
    def removeChain(self, ipv, table, chain, sender=None):
        ...
        if idx not in settings[0]:
            raise FirewallError(
                errors.NOT_ENABLED, "chain '%s' is not in '%s:%s'" % (chain, ipv, table)
            )
    ...
    def addRule(
        ...
        log.debug1(
            "config.direct.addRule('%s', '%s', '%s', %d, '%s')"
    ...
    def removeRule(
        ...
        args = dbus_to_python(args)
        log.debug1(
            "config.direct.removeRule('%s', '%s', '%s', %d, '%s')"
            % (ipv, table, chain, priority, "','".join(args))
        )
        self.accessCheck(sender)
        idx = (ipv, table, chain, priority, args)
    ...
    def queryRule(
        ...
        args = dbus_to_python(args)
        log.debug1(
            "config.direct.queryRule('%s', '%s', '%s', %d, '%s')"
            % (ipv, table, chain, priority, "','".join(args))
        )
        idx = (ipv, table, chain, priority, args)
    ...
    def removeRules(self, ipv, table, chain, sender=None):
        ...
        chain = dbus_to_python(chain)
        log.debug1(
            "config.direct.removeRules('%s', '%s', '%s')"
            % (
                ipv,
                table,
                chain,
    ...
    def getRules(self, ipv, table, chain, sender=None):  # pylint: disable=W0613
        ...
        table = dbus_to_python(table)
        chain = dbus_to_python(chain)
        log.debug1("config.direct.getRules('%s', '%s', '%s')" % (ipv, table, chain))
        ret = []
    ...
    def addPassthrough(self, ipv, args, sender=None):
        ipv = dbus_to_python(ipv)
        args = dbus_to_python(args)
        log.debug1("config.direct.addPassthrough('%s', '%s')" % (ipv, "','".join(args)))
        self.accessCheck(sender)
        idx = (ipv, args)
        settings = list(self.getSettings())
    ...
    def removePassthrough(self, ipv, args, sender=None):
        ...
        args = dbus_to_python(args)
        log.debug1(
            "config.direct.removePassthrough('%s', '%s')" % (ipv, "','".join(args))
        )
        self.accessCheck(sender)
        idx = (ipv, args)
        settings = list(self.getSettings())
    ...
    def queryPassthrough(self, ipv, args, sender=None):  # pylint: disable=W0613
        ...
        args = dbus_to_python(args)
        log.debug1(
            "config.direct.queryPassthrough('%s', '%s')" % (ipv, "','".join(args))
        )
        idx = (ipv, args)
