...
class FirewallDConfigIcmpType(DbusServiceObject):
    ...
    @dbus_service_method(
        config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, in_signature=IcmpType.DBUS_SIGNATURE
    )
    @dbus_handle_exceptions
    ...
    @dbus_service_method(config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, in_signature="s")
    @dbus_handle_exceptions
    ...
    @dbus_service_method(config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, out_signature="s")
    @dbus_handle_exceptions
    ...
    @dbus_service_method(config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, out_signature="as")
    @dbus_handle_exceptions
    ...
    @dbus_service_method(config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, in_signature="as")
    @dbus_handle_exceptions
    def setDestinations(self, destinations, sender=None):
        destinations = dbus_to_python(destinations, list)
        log.debug1(
            "%s.setDestinations('[%s]')", self._log_prefix, ",".join(destinations)
        )
        self.parent.accessCheck(sender)
        settings = list(self.getSettings())
    ...
    def addDestination(self, destination, sender=None):
        ...
        log.debug1("%s.addDestination('%s')", self._log_prefix, destination)
        self.parent.accessCheck(sender)
        settings = list(self.getSettings())
    ...
    def removeDestination(self, destination, sender=None):
        destination = dbus_to_python(destination, str)
        log.debug1("%s.removeDestination('%s')", self._log_prefix, destination)
        self.parent.accessCheck(sender)
        settings = list(self.getSettings())
        if settings[3]:
    ...
    @dbus_service_method(
        config.dbus.DBUS_INTERFACE_CONFIG_ICMPTYPE, in_signature="s", out_signature="b"
    )
    @dbus_handle_exceptions
